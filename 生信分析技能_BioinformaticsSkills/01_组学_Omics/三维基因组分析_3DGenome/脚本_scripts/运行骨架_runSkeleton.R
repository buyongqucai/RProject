# 三维基因组分析_3DGenome 骨架 status=skeleton
...
run_merged_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  pkgs <- c("GenomicInteractions", "ggplot2")
  ...
  message("【骨架】三维基因组分析_3DGenome input=", input_path, " out=", out_dir)
  invisible(list(status = "skeleton", folder = "三维基因组分析_3DGenome", pkgs_checked = present))
}
