# 空间转录组进阶 骨架 status=skeleton；包检查；出图 source 出版级出图
...
run_spatialadvanced_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
                  project_root = find_project_root()) {
  pkgs <- c("Seurat", "Giotto", "ggplot2")
  ...
  message("【骨架】空间转录组进阶 input=", input_path, " out=", out_dir)
  message("【骨架】CLI 期望: 无（见技能说明）")
  invisible(list(status = "skeleton", skill = "bioinfo-spatial-advanced", pkgs_checked = present, out_dir = out_dir))
}
if (sys.nframe() == 0L && !interactive()) message("加载 run_spatialadvanced_skeleton()")
