...
run_proteomics_pipeline <- function(quant_csv, sample_info_csv, out_dir,
                                    padj_cut = 0.05, logfc_cut = 0.58,
                                    project_root = find_project_root()) {
  pkgs <- c("limma", "ggplot2")
  check_r_packages(pkgs)
  source_pub_viz(project_root)
  stopifnot(file.exists(quant_csv), file.exists(sample_info_csv))
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  message("【骨架】定量=", quant_csv, " meta=", sample_info_csv)
  message("【骨架】TODO: 缺失过滤 -> 标准化 -> limma -> GSEA/PPI -> save_plot_pub")
  invisible(list(status = "skeleton", pkgs_ok = pkgs, out_dir = out_dir,
               thresholds = list(padj = padj_cut, logfc = logfc_cut)))
}

if (sys.nframe() == 0L && !interactive()) message("加载 run_proteomics_pipeline()")
