# 宏基因组与病原_Metagenome-Pathogen 骨架 status=skeleton
...
run_merged_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  message("【骨架】宏基因组与病原_Metagenome-Pathogen input=", input_path, " out=", out_dir)
  invisible(list(status = "skeleton", folder = "宏基因组与病原_Metagenome-Pathogen", pkgs_checked = present))
}
