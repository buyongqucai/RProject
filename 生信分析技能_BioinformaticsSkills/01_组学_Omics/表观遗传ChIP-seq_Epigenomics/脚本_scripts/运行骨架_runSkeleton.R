# 表观遗传ChIP-seq_Epigenomics 骨架 status=skeleton
...
run_merged_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  message("【骨架】表观遗传ChIP-seq_Epigenomics input=", input_path, " out=", out_dir)
  invisible(list(status = "skeleton", folder = "表观遗传ChIP-seq_Epigenomics", pkgs_checked = present))
}
