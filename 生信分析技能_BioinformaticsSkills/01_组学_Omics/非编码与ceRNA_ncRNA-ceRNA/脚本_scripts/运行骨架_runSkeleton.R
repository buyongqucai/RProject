# 非编码与ceRNA_ncRNA-ceRNA 骨架 status=skeleton
...
run_merged_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  pkgs <- c("clusterProfiler", "igraph", "ggplot2")
  ...
  message("【骨架】非编码与ceRNA_ncRNA-ceRNA input=", input_path, " out=", out_dir)
  invisible(list(status = "skeleton", folder = "非编码与ceRNA_ncRNA-ceRNA", pkgs_checked = present))
}
