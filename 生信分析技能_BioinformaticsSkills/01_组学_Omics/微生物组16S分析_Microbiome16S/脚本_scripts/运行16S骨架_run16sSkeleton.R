# 16S 骨架：包检查 + 出版级出图（QIIME2 为 CLI 备选）
# status = "skeleton"
...
run_16s_pipeline <- function(fastq_dir, sample_info_csv, out_dir,
                             trunc_len = c(240, 200),
                             project_root = find_project_root()) {
  pkgs <- c("dada2", "phyloseq", "vegan", "ggplot2")
  check_r_packages(pkgs)
  source_pub_viz(project_root)
  stopifnot(dir.exists(fastq_dir), file.exists(sample_info_csv))
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  message("【骨架】fastq=", fastq_dir, " truncLen=", paste(trunc_len, collapse = ","))
  message("【骨架】TODO: DADA2 -> phyloseq -> alpha/beta -> ANCOM-BC -> save_plot_pub")
  message("【骨架】QIIME2 备选见 脚本_scripts/说明_README.md")
  invisible(list(status = "skeleton", pkgs_ok = pkgs, out_dir = out_dir))
}

if (sys.nframe() == 0L && !interactive()) message("加载 run_16s_pipeline()")
