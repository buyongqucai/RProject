# 放射组学与病理组学 骨架 status=skeleton；包检查；出图 source 出版级出图
...
run_radiomics_pathomics_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  message("【骨架】放射组学与病理组学 input=", input_path, " out=", out_dir)
  message("【骨架】CLI 期望: 无（见技能说明）")
  invisible(list(status = "skeleton", skill = "bioinfo-radiomics", pkgs_checked = present, out_dir = out_dir))
}
if (sys.nframe() == 0L && !interactive()) message("加载 run_radiomics_pathomics_skeleton()")
