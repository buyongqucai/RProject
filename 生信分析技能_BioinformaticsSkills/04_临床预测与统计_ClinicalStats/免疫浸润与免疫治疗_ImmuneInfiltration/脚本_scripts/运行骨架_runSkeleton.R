# 免疫浸润与免疫治疗 骨架 status=skeleton；包检查；出图 source 出版级出图
...
run_immuneinfiltration_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  message("【骨架】免疫浸润与免疫治疗 input=", input_path, " out=", out_dir)
  message("【骨架】CLI 期望: CIBERSORTx")
  invisible(list(status = "skeleton", skill = "bioinfo-immune-infiltration", pkgs_checked = present, out_dir = out_dir))
}
if (sys.nframe() == 0L && !interactive()) message("加载 run_immuneinfiltration_skeleton()")
