# 剂量反应与时间序列_DoseTime 骨架 status=skeleton
...
run_merged_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  message("【骨架】剂量反应与时间序列_DoseTime input=", input_path, " out=", out_dir)
  invisible(list(status = "skeleton", folder = "剂量反应与时间序列_DoseTime", pkgs_checked = present))
}
