# 遗传关联下游_GeneticDownstream 骨架 status=skeleton
...
run_merged_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  message("【骨架】遗传关联下游_GeneticDownstream input=", input_path, " out=", out_dir)
  invisible(list(status = "skeleton", folder = "遗传关联下游_GeneticDownstream", pkgs_checked = present))
}
