# 孟德尔随机化 骨架 status=skeleton；包检查；出图 source 出版级出图
...
run_mendelianrandomization_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
                  project_root = find_project_root()) {
  pkgs <- c("TwoSampleMR", "MendelianRandomization", "ggplot2")
  ...
  message("【骨架】孟德尔随机化 input=", input_path, " out=", out_dir)
  message("【骨架】CLI 期望: 无（见技能说明）")
  invisible(list(status = "skeleton", skill = "bioinfo-mr", pkgs_checked = present, out_dir = out_dir))
}
if (sys.nframe() == 0L && !interactive()) message("加载 run_mendelianrandomization_skeleton()")
