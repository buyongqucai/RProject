# 变异检测与外显子组_Variant-WES 骨架 status=skeleton
...
run_merged_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  pkgs <- c("VariantAnnotation", "ggplot2")
  ...
  message("【骨架】变异检测与外显子组_Variant-WES input=", input_path, " out=", out_dir)
  invisible(list(status = "skeleton", folder = "变异检测与外显子组_Variant-WES", pkgs_checked = present))
}
