# 群体遗传学 骨架 status=skeleton；包检查；出图 source 出版级出图
...
run_populationgenetics_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  message("【骨架】群体遗传学 input=", input_path, " out=", out_dir)
  message("【骨架】CLI 期望: PLINK, ADMIXTURE")
  invisible(list(status = "skeleton", skill = "bioinfo-popgen", pkgs_checked = present, out_dir = out_dir))
}
if (sys.nframe() == 0L && !interactive()) message("加载 run_populationgenetics_skeleton()")
