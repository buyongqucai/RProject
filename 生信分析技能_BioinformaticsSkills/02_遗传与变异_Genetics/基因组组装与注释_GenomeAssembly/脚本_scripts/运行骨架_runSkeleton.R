# 基因组组装与注释 骨架 status=skeleton；包检查；出图 source 出版级出图
...
run_genomeassembly_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
                  project_root = find_project_root()) {
  pkgs <- c("rtracklayer", "ggplot2")
  ...
  message("【骨架】基因组组装与注释 input=", input_path, " out=", out_dir)
  message("【骨架】CLI 期望: SPAdes, Flye, Prokka")
  invisible(list(status = "skeleton", skill = "bioinfo-genome-assembly", pkgs_checked = present, out_dir = out_dir))
}
if (sys.nframe() == 0L && !interactive()) message("加载 run_genomeassembly_skeleton()")
