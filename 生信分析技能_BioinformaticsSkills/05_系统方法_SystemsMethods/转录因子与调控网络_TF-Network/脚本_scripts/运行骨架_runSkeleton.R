# 转录因子与调控网络 骨架 status=skeleton；包检查；出图 source 出版级出图
...
run_tf_network_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
                  project_root = find_project_root()) {
  pkgs <- c("decoupleR", "ggplot2")
  ...
  message("【骨架】转录因子与调控网络 input=", input_path, " out=", out_dir)
  message("【骨架】CLI 期望: HOMER")
  invisible(list(status = "skeleton", skill = "bioinfo-tf-network", pkgs_checked = present, out_dir = out_dir))
}
if (sys.nframe() == 0L && !interactive()) message("加载 run_tf_network_skeleton()")
