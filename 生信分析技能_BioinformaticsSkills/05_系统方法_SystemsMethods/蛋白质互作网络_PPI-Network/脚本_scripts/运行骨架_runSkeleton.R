# 蛋白质互作网络 骨架 status=skeleton；包检查；出图 source 出版级出图
...
run_ppi_network_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
                  project_root = find_project_root()) {
  pkgs <- c("STRINGdb", "igraph", "ggplot2")
  ...
  message("【骨架】蛋白质互作网络 input=", input_path, " out=", out_dir)
  message("【骨架】CLI 期望: Cytoscape")
  invisible(list(status = "skeleton", skill = "bioinfo-ppi", pkgs_checked = present, out_dir = out_dir))
}
if (sys.nframe() == 0L && !interactive()) message("加载 run_ppi_network_skeleton()")
