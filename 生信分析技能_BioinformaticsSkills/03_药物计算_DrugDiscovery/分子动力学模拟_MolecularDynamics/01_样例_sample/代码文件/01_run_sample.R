...
md_anal <- file.path(sample_root, "工作文件_MdWork", "3HTB", "08_轨迹分析_Analysis")
need <- file.path(md_anal, c(
  "骨架RMSD_RmsdBackbone.xvg",
  "残基RMSF_RmsfCalpha.xvg",
  "氢键数_HbondNum.xvg",
  "回旋半径_RadiusGyration.xvg"
))

if (!all(file.exists(need))) {
  message("MD 轨迹分析输入未就绪。请先在 WSL2 跑流水线（幂等，断点续跑），再归档：")
  message('  wsl -d Ubuntu-24.04 -- bash -c "cd <扁平工作目录> && \\')
  message('    LIGAND_RES=JZ4 NET_CHARGE=0 NS=0.2 \\')
  message('    bash <技能根>/脚本_scripts/运行复合物MD_runComplexMd.sh . protein.pdb jz4_h.mol2"')
  message('  python <技能根>/脚本_scripts/整理样例目录_layoutMdSample.py')
  quit(save = "no", status = 1)
}

source(file.path("02_分析出图_plotMdAnalysis.R"), encoding = "UTF-8")
