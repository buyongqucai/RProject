# 分子对接与虚拟筛选 骨架 status=skeleton；包检查；出图 source 出版级出图
...
run_moleculardocking_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
                  project_root = find_project_root()) {
  pkgs <- c("bio3d", "ggplot2")
  ...
  message("【骨架】分子对接与虚拟筛选 input=", input_path, " out=", out_dir)
  message("【骨架】CLI 期望: AutoDock Vina, Open Babel")
  invisible(list(status = "skeleton", skill = "bioinfo-molecular-docking", pkgs_checked = present, out_dir = out_dir))
}
if (sys.nframe() == 0L && !interactive()) message("加载 run_moleculardocking_skeleton()")
