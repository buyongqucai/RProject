# 蛋白结构与建模_ProteinStructure 骨架 status=skeleton
...
run_merged_skeleton <- function(input_path = NULL, out_dir = tempfile("bioinfo_"),
  ...
  message("【骨架】蛋白结构与建模_ProteinStructure input=", input_path, " out=", out_dir)
  invisible(list(status = "skeleton", folder = "蛋白结构与建模_ProteinStructure", pkgs_checked = present))
}
